...
const positionAcc = [1, 1, 1, 1]
const positionDebounce = [_.debounce(() => {
    positionAcc[0] = 1
}, 100), _.debounce((index) => {
    positionAcc[1] = 1
}, 100), _.debounce((index) => {
    positionAcc[2] = 1
}, 100), _.debounce((index) => {
...
// 全局监听按键操作并执行相应命令
export function listenGlobalKeyDown(_this) {
    window.onkeydown = (e) => {
    if (!store.state.isInEdiotr) return

    const { curComponent } = store.state
    const { keyCode } = e
    if (keyCode == deleteKey && curComponent) {
    ...
    //     } else if (keyCode == 38) {
    //         // 修改位置
    //         if (_this.$store.state.curComponent) {
    //             _this.$store.state.curComponent.style.top -= positionAcc[0]
    //             positionAcc[0] += 2
    //             positionDebounce[0]()
    //         }
    //     } else if (keyCode == 40) {
    //         if (_this.$store.state.curComponent) {
    //             _this.$store.state.curComponent.style.top += positionAcc[1]
    //             positionAcc[1] += 2
    //             positionDebounce[1]()
    //         }
    //     } else if (keyCode == 37) {
    //         if (_this.$store.state.curComponent) {
    //             _this.$store.state.curComponent.style.left -= positionAcc[2]
    //             positionAcc[2] += 2
    //             positionDebounce[2]()
    //         }
    //     } else if (keyCode == 39) {
    //         if (_this.$store.state.curComponent) {
    //             _this.$store.state.curComponent.style.left += positionAcc[3]
    //             positionAcc[3] += 2
    //             positionDebounce[3]()
    //         }
    //     }
