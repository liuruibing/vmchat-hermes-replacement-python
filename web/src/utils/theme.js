...
export default function changeTheme(val) {
  if (typeof val !== 'string') return
  const themeCluster = getThemeCluster(val.replace('#', ''))
  const originalCluster = getThemeCluster(theme.replace('#', ''))
  ...
  const styles = [].slice.call(document.querySelectorAll('style'))
    .filter(style => {
      ...
      return new RegExp(theme, 'i').test(text) && !/Chalk Variables/.test(text)
    })
