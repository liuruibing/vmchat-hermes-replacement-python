            ...
            <el-col :span="16">
              <div class="grid-content bg-purple">
                <el-form-item label="开始日期" prop="beginDate">
                  <el-date-picker v-model="formInline.beginDate" :picker-options="pickerOptions" placeholder="选择开始日期" type="date" value-format="yyyy-MM-dd" />
                </el-form-item>
                <el-form-item label="结束日期" prop="endDate">
                  <el-date-picker v-model="formInline.endDate" :picker-options="pickerOptions" placeholder="选择结束日期" type="date" value-format="yyyy-MM-dd" />
                </el-form-item>
            ...
            <el-col :span="8">
              <div class="grid-content bg-purple-light">
                <el-form-item class="search_button_com">
                  <el-button icon="el-icon-search" size="small" style="margin-bottom: 5px" type="primary" @click="onSubmit('formInline')">
                    查询
                  </el-button>
                  <el-button icon="el-icon-refresh-left" plain size="small" type="info" @click="resetForm('formInline')">
                    重置
                  </el-button>
                  <!-- <el-button
                  ...
                  <el-button type="primary" plain :icon="exportIcon2" style="margin-bottom: 5px" :disabled="exportButton2" @click="fun_export_PDF" size="small">
                    {{ exportText2 }}
                  </el-button>
                </el-form-item>
              </div>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      name: 'iframeVue_reportSYCPBG',
      id: 'J_iframe_id_reportSYCPBG',
      baseApi: '', // 页面根地址
      exportButton: false,
      exportText: '导出',
  ...
  computed: {
    ...mapState({
      fromPageJumpSYCP: state => state.user.fromPageJumpSYCP // 将 store 中的变量映射到组件的计算属性中
    })
  },
  created() {
    this.$bus.$on('fromPageJumpSYCP', row => {
      this.formInline.beginDate = this.getYearFirstDayAndYesterday().yearFirstDay
      this.formInline.endDate = this.getYearFirstDayAndYesterday().yesterday
      this.formInline.fundCode = row.fundCode
      this.formInline.fundName = row.fundName
      this.selectList = [
        {
  ...
  methods: {
    getYearFirstDayAndYesterday() {
      const currentDate = new Date()
      const currentYear = currentDate.getFullYear()

      // 获取今年的第一天
      const yearFirstDay = new Date(currentYear, 0, 1)
      const formattedYearFirstDay = this.formatDate(yearFirstDay)

      // 获取当前时间的前一天
      const yesterday = new Date(currentDate)
      yesterday.setDate(currentDate.getDate() - 1)

      // 如果是今年的第一天，则将前一天设置为今年的第一天
      if (currentDate.getTime() === yearFirstDay.getTime()) {
        yesterday.setTime(yearFirstDay.getTime())
      }

      const formattedYesterday = this.formatDate(yesterday)

      return {
        yearFirstDay: formattedYearFirstDay,
    ...
    fun_iframeSrc() {
      ...
      try {
        self.iframeData.src = this.file_vmrisk_src(
          ...
          '13cf6177-27fc-44a8-bfb5-43b4a71aac1c',
    ...
    fun_export_PDF() {
      ...
      const templateCode = '13cf6177-27fc-44a8-bfb5-43b4a71aac1c'
      const wordName = '私有产品报告.docx'
