<template>
  <div>
          ...
          <el-col :span="4">
            <div class="grid-content bg-purple-light">
              <el-form-item class="search_button_com">
                ...
                <!-- <el-button type="primary" plain @click="downloadData()" style="margin-bottom: 5px; " size="small"><i
    ...
    <div class="forms_table_title">
      <i class="el-icon-s-data"></i>基金管理规模
    </div>
    <!-- sortable='custom' -->
    <el-table ref="multipleTable" @selection-change="handleSelectionChange" :data="tableData" :default-sort="{prop: 'FUNDNAME', order: 'descending'}"
      border stripe @sort-change="handleSortChange" v-loading="tableLoading" style="width: 100%">
      <el-table-column type="index" label="序号" fixed align="center" width="50">
      </el-table-column>


 <el-table-column prop="FUNDCODE"   label="基金代码" :sort-orders="['ascending', 'descending']" min-width="150"
        header-align="left" align="left">
        <template slot-scope="scope">
          <span>{{scope.row.FUNDCODE | noDataFilter}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="FUNDNAME"   label="基金名称" :sort-orders="['ascending', 'descending']" min-width="150"
        header-align="left" align="left">
        <template slot-scope="scope">
      ...
      <el-table-column prop="CREATEDATE"   label="成立日期" :sort-orders="['ascending', 'descending']" min-width="150"
        header-align="left" align="left">
        <template slot-scope="scope">
      ...
      <el-table-column prop="FELB"   label="基金类别" :sort-orders="['ascending', 'descending']" min-width="150" header-align="left"
        align="left">
        <template slot-scope="scope">
          <span>{{scope.row.FELB | noDataFilter}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="YMJZC"   label="月末资产净值(元)" :sort-orders="['ascending', 'descending']" min-width="150"
        header-align="left" align="left">
        <template slot-scope="scope">
          <span>{{scope.row.YMJZC | moneyThousand}}</span>
        </template>
      </el-table-column>

      <el-table-column prop="JJGLF"   label="基金管理费本年累计(元)" :sort-orders="['ascending', 'descending']" min-width="200"
        header-align="left" align="left">
        <template slot-scope="scope">
          <span>{{scope.row.JJGLF | moneyThousand}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="JJGLFL"   label="基金管理费率" :sort-orders="['ascending', 'descending']" min-width="150"
        header-align="left" align="left">
        <template slot-scope="scope">
          <span>{{scope.row.JJGLFL | noDataFilter}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="YZTS"   label="本年运作天数(天)" :sort-orders="['ascending', 'descending']" min-width="150"
        header-align="left" align="left">
        <template slot-scope="scope">
          <span>{{scope.row.YZTS | noDataFilter}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="RJGLGM"   label="本年基金日均管理规模(元)" :sort-orders="['ascending', 'descending']" min-width="200"
        header-align="left" align="left">
        <template slot-scope="scope">
          <span>{{scope.row.RJGLGM | moneyThousand}}</span>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  ...
  import pageApi from "@/api/reportCenter/fundManageScale";
  import $ from 'jquery'
  export default {
    ...
    methods: {
      ...
      downloadData() {
        ...
        let params = {};
        // 选择月份
        if (this.formInline.tDate) {
          params["tDate"] = this.formInline.tDate;
        }
        // token信息
        ...
        // 表头名
        params["exportAliasName"] = '基金名称,成立日期,基金类别,月末资产净值(元),基金管理费本年累计(元),基金管理费率,本年运作天数(天),本年基金日均管理规模(元)';
        // 导出文件名
