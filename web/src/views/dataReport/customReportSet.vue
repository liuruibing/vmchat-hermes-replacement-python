<template>
   <div>
  <el-form :inline='true' :rules="rulesForm" ref='formInline' :model='formInline' @submit.native.prevent style='margin-bottom: 8px;' class='demo-form-inline form_inline_search' >
    <el-row>
      <el-col :span='16'>
         <div style='width:100%;height:50px;'>
            <el-form-item label='报表名称' prop='reportName'>
                <el-input maxlength="33" type='text' v-model.trim='formInline.reportName' clearable placeholder='请输入名称' size='small' @keydown.enter.native="onSubmit(true)"></el-input>
            </el-form-item>
            <el-form-item label="是否开放" prop="status">
                <el-select clearable  v-model="formInline.status"  size="small"  placeholder="请选择"  >
                  <el-option
                    v-for="(group, index) in statusArr"
                    :value="group.value"
                    :key="index"
                    :label="group.key">
                  </el-option>
...
export default {
   ...
   data () {
      return {
        rulesForm: {
          reportName: [
            { required: false, type: 'string', message: "请输入报表名称", trigger: "change" },
            { pattern: this.$regularValidator.empty().reg, message: this.$regularValidator.empty().msg }
          ]
        },
        rulesFormVisit: {
          fundCode: [
            { required: false, type: 'string', message: "请输入产品代码", trigger: "change" },
            { pattern: /^[a-zA-Z0-9]+$/, message: '请输入英文或者数字' }
          ]
        },
         formInline:{
            reportName:'',
            ...
            pageNum:1
		     ...
		     statusArr:[{key:"是",value:"1"},{key:"否",value:"0"}],
         tableData: [], //表格数据
         ...
         countDialog: false, // 弹窗
          ...
          countRules: {
             reportName: [
              ...
              remark: [
                { required: false, pattern: this.$regularValidator.empty().reg, trigger: "change", message: this.$regularValidator.empty().msg }
              ]
          },
   ...
   methods: {
    ...
    onSubmit(flag){
      this.$refs['formInline'].validate((valid) => {
        if (valid) this.passValidate(flag)
        else {
    ...
    passValidate (flag) {
      this.tableLoading = true;
        ...
        pageApi.getDataSourceListAll(params).then(response => { 
    ...
    //新增数据
      ...
      searchData(row){
          ...
          //  this.report(true);
          this.passValidatereport(true)
            
      },
     report(flag){
        this.$refs['formInlineDetail'].validate((valid) => {
          if (valid) this.passValidatereport(flag)
          else {
            this.$message.closeAll()
            this.$message.error('请输入正确的查询条件 ！')
          }
