<template>
  <div class="currency_report_old vxe_custom_style">
        ...
        <el-col :span="18">
          <div style="width: 100%; min-height: 50px">
            ...
            >
              ...
              <el-select
                v-if="item.dicSql && !item.formatValue && !item.validateRule"
                clearable
                filterable
                v-model="formInline[item.fieldEn]"
                size="small"
              ...
              <!-- 父 -->
              <el-select
                v-if="item.dicSql && item.formatValue"
                clearable
                filterable
                v-model="formInline[item.fieldEn]"
                size="small"
                placeholder="请选择"
                @change="getSunchange(item.fieldEn,item.formatValue)"
              >
                <el-option
                  v-for="(group, index) in formSelectArr[item.fieldEn]"
                  :value="group.DIM_CDE"
                  :key="index"
                  :label="group.DIM_NME"
                  @click.native="getSun(group.DIM_CDE,group.ID,item.fieldEn,item.formatValue)"
                >
                </el-option>
              </el-select>
    ...
    <div v-loading="tableLoading" style="">
      ...
      <div class="vxetable-style" style="height:calc(100vh - 284px)">
        <vxe-toolbar
          v-if="tableShow"
          custom
...
export default {
  ...
  methods: {
    ...
    getSun(val,pId,p_field,c_field){
      // console.log(this.formInline[c]);
      if(this.formInline[c_field]){
        this.formInline[c_field] = '';
      }
      // console.log(this.formSelectArr);
      // console.log(val,pId,p_field,c_field);
    ...
    getSunchange(data,c_field){
      // console.log(data,c_field);
