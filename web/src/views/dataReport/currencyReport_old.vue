<template>
  <div class="currency_report_old vxe_custom_style">
    <el-tabs
    ...
    <el-form
      ...
      class="demo-form-inline form_inline_search currency_report_old_form labelStyle"
    >
      <div class="top_box">
        <div class="top_box_l">
          <div style="width: 100%; min-height: 50px">
            <el-form-item
              ...
              :label="item.fieldCn"
              :prop="item.fieldEn"
              :label-width="index > 0 ? '144px' : '144px'"
            >
              <span slot="label" :title="item.fieldCn">{{item.fieldCn}}</span>
              <el-input
              ...
              <!-- <el-select
                v-if="item.dicSql && !item.sonEn && !item.validateRule"
                clearable
                filterable
                v-model="formInline[item.fieldEn]"
                size="small"
                placeholder="请选择"
                @change="selectChange($event,item.fieldEn)"
              >
                <el-option
              ...
              <!-- 子 -->
              <!-- <el-select
                v-if="item.dicSql && item.validateRule"
                clearable
                filterable
                v-model="formInline[item.fieldEn]"
                size="small"
    ...
    <div  style="">
      <div class="forms_table_title" style="float: left">
        ...
        <span style="margin-left:20px">
            ...
            <el-button v-if="crudEnable.indexOf('B')!=-1" type="primary" plain @click="print('pdf')" style="margin-bottom: 5px; " size="small">
              <i class="el-icon-printer"></i>打印
            </el-button>
            <el-button v-if="crudEnable.indexOf('B')!=-1" type="primary" plain @click="mailmail()" style="margin-bottom: 5px; " size="small">
              <i class="el-icon-message"></i>快捷邮件发送
            </el-button>
            <el-button v-if="crudEnable.indexOf('B')!=-1" type="primary" plain @click="customMail()" style="margin-bottom: 5px; " size="small">
              <i class="el-icon-message"></i>自定义邮件发送
            </el-button>
          </span>
      </div>

      <div class="vxetable-style" :style="{height:`calc(100vh - ${searchHeight}px)`}">
        <vxe-toolbar
          v-if="tableShow"
          custom
        ...
        >
          ...
          <template slot="default" slot-scope="{row}">
              ...
              <!-- <template v-if="group.fieldType=='N'">
                <span>{{amountFilter(row[group.fieldEn],group.formatValue)}}</span>
              </template> -->
          ...
          <vxe-table-column
            fixed="right"
            v-if="
              crudEnable.indexOf('U') != -1 || crudEnable.indexOf('D') != -1 || reportCode == 'id-9292d1d-22fa-09d6-b7e6-c61e4146b13f'
            "
            title="操作"
            width="200"
            show-overflow
          ...
          >
            <template slot-scope="scope">
              <el-button
                v-if="reportCode == 'id-9292d1d-22fa-09d6-b7e6-c61e4146b13f'"
                size="small"
                @click="detailData(scope.row, true)"
                >查看明细</el-button
              >
              <el-button
          ...
          >
            ...
            >
              ...
              <el-select
                ...
                size="small"
                placeholder="请选择"
                @change="item.sonEn?editGetSunchange($event,item.fieldEn,item.sonEn):''"
              >
                <el-option
                  ...
                  :key="index"
                  :label="group.DIM_NME"
                  @click.native="item.sonEn?editGetSun(group.DIM_CDE,group.DIM_NME,group.ID,item.fieldEn,item.sonEn):''"
                >
                </el-option>
              </el-select>
              <!-- 日期 yyyy-MM-dd -->
    ...
    >
    <mySelf v-if="detail_show" :currencyReportPageData="currencyReportPageData_dialog"></mySelf>
    </el-dialog>
    <el-dialog
      title="快捷邮件发送"
      append-to-body
    ...
    >
      ...
      >
        ...
        >
          <template slot-scope="scope">
            ...
            <el-radio v-model="mailId" :label="scope.row.id">{{scope.row.confName}}</el-radio>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="small" @click="mailDialog = false" :loading="send_loading">取 消</el-button>
        <el-button
          type="primary"
          size="small"
          @click="sendMail()"
...
import reportToolApi from "@/api/reportConfig/reportTool";
import commonApi from "@/api/common";
import mySelf from "./currencyReport_old_copy.vue"
import sendMailForm from "./sendMailForm.vue"
export default {
  name: "currencyReport2",
  components: { Pagination,mySelf,sendMailForm,selectTable,multipleSelectTable },
  props: {
  ...
  methods: {
    ...
    fetTableHeight(){
      ...
      if(h&&!tabsH){
        this.searchHeight = h+240;
      }
      if(h&&tabsH){
        this.searchHeight = h+tabsH+240;
      }
      if(!h&&tabsH){
        this.searchHeight = tabsH+240;
      }
    ...
    handleClick(tab, event) {
      ...
      this.searchDataArr = [];
      this.tableDataDetailArr = this.getAllPrpos(this.tabInfo[0][tab.name])
    ...
    resetForm(){
      ...
      // this.$nextTick(()=>{
        this.searchDataArr.forEach(item=>{
          if(item.inputType=='select_multiple' || item.inputType=='select_multiple_fund'){
        ...
        .then((response) => {
          ...
          if (response.data.status === 200) {
            ...
            if(response.data.timestamp){
              this.timestamp = commonFun.yyr_timestampToTime(response.data.timestamp);
            ...
            if(data[0]){
              ...
              this.tableDataDetailArr = this.getAllPrpos(data[0][arr[0].tableCode]);
              this.tableShow = true;
              this.tableLoading = false;
            }
    ...
    editGetSunchange(val,fieldEn,c_field){
      // console.log('父级下拉框-change',val,fieldEn,c_field);
      if(!val){
        this.$set(this.editFormSelectArr,c_field,this.editFormSelectArr2[c_field]);
        // this.formCount[c_field] = '';
      }
    },
    // 新增编辑下拉框关闭
    editFormClose(){
      this.editFormSelectArr = JSON.parse(JSON.stringify(this.editFormSelectArr2));
    },
    ...
    mailmail(){
      ...
      reportToolApi.mailList(params).then(res=>{
...
.select_multiple_popper.el-select-dropdown.is-multiple .el-select-dropdown__item.selected::after{
  right:0;
  left: 2px;
}
// 查询条件
