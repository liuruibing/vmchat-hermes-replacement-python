<template>
  <div id="moreQuery">
    <el-form ref="queryForm" :inline="true" :model="clsjSection" novalidate label-position="left" size="small">
      <!-- 已选条件 -->
      <el-row class="yxtj" style="margin-bottom: 10px;">
        <el-form-item label="已选条件" label-width="80px" style="display:flex; align-items:center;margin-bottom: 0px!important;">
          <div class="tag-list">
      ...
      <!-- 投资策略 -->
      <el-row class="tzcl" style="margin-bottom: 10px; border-bottom: 1px solid #eee; padding-bottom: 5px;">
        <el-form-item label="投资策略" label-width="80px" style="display:block; align-items:center">
          <div style="display: flex; flex-wrap: wrap; gap: 15px;">
            <span class="tzcl-text" @click="handleBxBtn('.tzclbx-focus')" :class="{ 'active': !investStrategy.some(item => item.checked) }">
              不限
            </span>
            <span
              ...
              :key="ele.id"
              class="tzcl-text"
              @click="handleStrategyClick(ele)"
              @mouseenter="handleFirstLevelHover(ele)"
              @mouseleave="handleFirstLevelLeave"
              :class="{ 'active': ele.checked, 'hover': hoverFirstId === ele.id }">
              {{ ele.label }}
            </span>
          </div>

          <!-- 二级策略显示区域 -->
          <div v-if="showSubStrategy" class="strategy-level-box" @mouseenter="keepSubStrategyShow" @mouseleave="handleSubLevelLeave">
            <div style="display: flex; flex-wrap: wrap; gap: 15px;">
              <span class="tzsubcl-text">子策略：</span>
              <span class="tzsubcl-text" @click="handleSubBxBtn('.tzsubclbx-focus')" :class="{ 'active': !selectedSubStrategyIds.length }">
                不限
              </span>
              <span
                v-for="ele in currentFirstSubStrategy"
                :key="ele.id"
                class="tzsubcl-text"
                @click="handleSubStrategyClick(ele)"
                @mouseenter="handleSecondLevelHover(ele)"
                @mouseleave="handleSecondLevelLeave"
                :class="{ 'active': ele.checked, 'hover': hoverSecondId === ele.id }">
                {{ ele.label }}
              </span>
            </div>
            <!-- 三级策略显示区域 -->
            <div v-if="showThirdStrategy" class="strategy-level-box third-level" @mouseenter="keepThirdStrategyShow" @mouseleave="handleThirdLevelLeave">
              <div style="display: flex; flex-wrap: wrap; gap: 15px;">
                <span class="tzthirdcl-text">三级策略：</span>
                <span class="tzthirdcl-text" @click="handleThirdBxBtn('.tzthirdclbx-focus')" :class="{ 'active': !currentSecondThirdStrategy.some(item => item.checked) }">
                  不限
                </span>
                <span
                  v-for="ele in currentSecondThirdStrategy"
                  :key="ele.id"
                  class="tzthirdcl-text"
                  @click="handleThirdStrategyClick(ele)"
      ...
      <el-row class="gdzb" style="margin-bottom: 10px;">
        <el-form-item label="更多指标" label-width="80px" style="display:block; align-items:center">
          <div style="display: flex; flex-wrap: wrap; gap: 15px;">
            ...
            <span class="tzcl-text" @mouseenter="toggleMoreIndicator('isClassified')"  @click="toggleMoreIndicator('isClassified')" :class="{ 'active':  dynamicTags.some(tag => tag.id === '3') }">
              是否分级
            </span>
            ...
            <span class="tzcl-text" @mouseenter="toggleMoreIndicator('netValueFrequency')"  @click="toggleMoreIndicator('netValueFrequency')" :class="{ 'active':  dynamicTags.some(tag => tag.id === '4') }">
              净值频率
            </span>
          </div>
          ...
          <div v-if="activeMoreIndicator" class="strategy-level-box">
            ...
            <!-- 是否分级 -->
            <div v-if="activeMoreIndicator === 'isClassified'" style="margin-top: 0px;">
              <div style="display: flex; gap: 15px;">
                <span class="tzsubcl-text" >是否分级：</span>
                <span class="tzsubcl-text" @click="selectClassified(2)" :class="{ 'active': sffjData[0].checked === 2 }">不限</span>
                <span class="tzsubcl-text" @click="selectClassified(1)" :class="{ 'active': sffjData[0].checked === 1 }">分级</span>
                <span class="tzsubcl-text" @click="selectClassified(0)" :class="{ 'active': sffjData[0].checked === 0 }">不分级</span>
              </div>
            </div>
            ...
            <!-- 净值频率 -->
            <div v-if="activeMoreIndicator === 'netValueFrequency'" style="margin-top: 0px;">
              <div style="display: flex; gap: 15px;">
                <span class="tzsubcl-text" >净值频率：</span>
                <span class="tzsubcl-text" @click="selectFrequency(3)" :class="{ 'active': jzplOptions[0].selected === 3 }">不限</span>
                <span class="tzsubcl-text" @click="selectFrequency(0)" :class="{ 'active': jzplOptions[0].selected === 0 }">日频</span>
                <span class="tzsubcl-text" @click="selectFrequency(1)" :class="{ 'active': jzplOptions[0].selected === 1 }">周频</span>
                <span class="tzsubcl-text" @click="selectFrequency(2)" :class="{ 'active': jzplOptions[0].selected === 2 }">月频</span>
              </div>
            </div>
          </div>
        </el-form-item>
  ...
  methods: {
    ...
    getInvestSubStrategyList(param) {
      request({
        ...
        if (res.data.status === 0) {
          this.investSubOptions = res.data.data || []
        }
      })
    },
    getInvestThirdStrategyList(param) {
      request({
        ...
        if (res.data.status === 0) {
          this.investThirdOptions = res.data.data || []
        }
      })
    },
    filterData(id, name) {
