<template>
  <el-dialog :visible.sync="productSelectDialogVisible" append-to-body width="80%" @close="handleDialogClose">
          ...
          <draggable v-model="selectListDialog" @end="dragging = false" @start="dragging = true">
            <div v-for="element in selectListDialog" :key="element.id" class="drag-element-style" style="line-height: 30px">
              <el-row align="middle" type="flex">
                <el-col :span="20" style="padding: 0 10px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap" :title="element.VC_FUNDNAME">
                  {{ element.VC_FUNDNAME }}
                </el-col>
    ...
    <div slot="footer" class="dialog-footer-center standard-form">
      <el-button size="small" type="info" plain @click="handleDialogClose" :disabled="disabled">取 消</el-button>
      <el-button size="small" type="primary" @click="handleSubmit" :disabled="disabled">确定</el-button>
    </div>
  </el-dialog>
</template>
<script>
  ...
  methods: {
    ...
    judgeFlag(id) {
      ...
      // 判断是投后的穿透页面的
        ...
        else if (this.$route.path.includes('/thgl/thyj/yjgzgl')) {
        if (id == 'zstgwb') {
          return 'B'
        } else if (id == 'wdgz') {
          return 'B'
      ...
      // 判断是标准化需求管理下的页面
      else if (this.$route.path.includes('/xqhd/bzhxqgl')) {
        if (id == 'scsm') {
          return ''
        } else if (id == 'zstgwb') {
          return 'B'
