<template>
  <div id="moreQuery">
    <el-form ref="queryForm" :inline="true" :model="clsjSection" :rules="searchRules" label-position="left" size="small">
      <!-- 投资策略 -->
      <el-row class="tzcl">
        <el-form-item label="投资策略" label-width="70px" style="display:flex; align-items:center">
          <span class="tzclbx-focus" style="font-size: 14px;padding: 4px 10px" @click="handleBxBtn('.tzclbx-focus')">
            不限
          </span>
      ...
      <el-row class="tzsubcl" v-show="isSubStrategyVisible">
        <el-form-item label="子策略" label-width="70px" style="display:flex; align-items:center">
          <span class="tzsubclbx-focus" style="font-size: 14px;padding: 4px 10px" @click="handleSubBxBtn('.tzsubclbx-focus')">
            不限
          </span>
      ...
      <el-row class="clsj">
        ...
        <el-form-item v-for="b in ljjz" :key="b.mid" :label="b.title" :label-width="b.width" style="display:flex; align-items:center">
          <el-input v-model="b.start" onkeyup="value=value.replace(/[^0-9.-]/g,'').replace(/^(\-)*(\d+)\.(\d\d).*$/,'$1$2.$3')" size="small" style="width: 72px!important" />
          <span>-</span>
          <el-input v-model="b.end" onkeyup="value=value.replace(/[^0-9.-]/g,'').replace(/^(\-)*(\d+)\.(\d\d).*$/,'$1$2.$3')" size="small" style="width: 72px!important" />
          <span :class="b.style ? 'submit-icon' : 'submit-icon-disabled'">
            <el-button circle icon="el-icon-check" type="gfcheck" @click="addTagInput(b, '20')"></el-button>
          </span>
        </el-form-item>
        <el-form-item v-for="b in zcjzs" :key="b.mid" :label="b.title" :label-width="b.width" style="display:flex; align-items:center">
          <el-input v-model="b.start" onkeyup="value=value.replace(/[^0-9.-]/g,'').replace(/^(\-)*(\d+)\.(\d\d).*$/,'$1$2.$3')" size="small" style="width: 72px!important" />
          <span>-</span>
          <el-input v-model="b.end" onkeyup="value=value.replace(/[^0-9.-]/g,'').replace(/^(\-)*(\d+)\.(\d\d).*$/,'$1$2.$3')" size="small" style="width: 72px!important" />
          <span>(万元)</span>
          <span :class="b.style ? 'submit-icon' : 'submit-icon-disabled'">
            <el-button circle icon="el-icon-check" type="gfcheck" @click="addTagInput(b, '21')"></el-button>
          </span>
        </el-form-item>
      </el-row>
        ...
        <el-form-item label="成立以来大类资产占比(平均值)" style="display:flex; align-items:center">
          ...
          >
            ...
            <div v-show="item.isShow" class="hover-item">
              ...
              <span :class="item.style ? 'submit-icon' : 'submit-icon-disabled'">
                <el-button
                  circle
                  icon="el-icon-check"
                  type="primary"
                  @click="addTagInput(item, item.id)"
                ></el-button>
              </span>
            </div>
          </el-radio>
...
export default {
  ...
  methods: {
    ...
    /**
     * @description 子策略点击不限取消所有checkbox选中
     */
    handleSubBxBtn(val) {
      // 获取对应dom节点修改样式
      const Dom = document.querySelector(val)
      ...
      if (isVaildate) {
        ...
        if (_index < 0) {
          ...
          if (id === '7') {
            ...
            this.ljjz[0].style = true
          } else if (id === '21') {
            this.zcjzs[0].style = true
          } else if (id === '15') {
            this.sinceThe[0].style = true
          } else if (id === '16') {
            this.sinceThe[1].style = true
          } else if (id === '17') {
            this.sinceThe[2].style = true
          } else if (id === '18') {
            this.sinceThe[3].style = true
          }
        } else {
          ...
          if (id === '7') {
            ...
            this.sinceThe[0].style = false
          } else if (id === '16') {
            this.sinceThe[1].style = false
          } else if (id === '17') {
            this.sinceThe[2].style = false
          } else if (id === '18') {
            this.sinceThe[3].style = false
          }
        }
      }
    },
