...
export default {
  ...
  data() {
    ...
    return {
      ...
      //开关
      offOn: false,
      name: 'iframeVue_reportGF_DYZBG',
      id: 'J_iframe_id_reportGF_DYZBG',
      baseApi: '', // 页面根地址
      exportButton: false,
      exportText: '导出',
  ...
  watch: {
    '$route.path'(val) {
      if (val == '/jxbg/dyzbg' && this.$store.getters.isHome === true) {
  ...
  methods: {
      ...
      try {
        if (this.reportType == 'preview') {
          self.iframeData.src = this.file_vmrisk_src_preview(
            ...
            '3ABE40895CD0FC9DE063229C500A0C6A',
    ...
    fun_export_PDF() {
      ...
      const templateCode =  '3ABE40895CD0FC9DE063229C500A0C6A'
      const pdfName = '多因子报告.pdf'
