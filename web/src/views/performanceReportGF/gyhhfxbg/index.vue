...
export default {
  ...
  data() {
    ...
    return {
      ...
      //开关
      offOn: false,
      name: 'iframeVue_reportGF_GYHHFXBG',
      id: 'J_iframe_id_reportGF_GYHHFXBG',
      baseApi: '', // 页面根地址
      exportButton: false,
      exportText: '导出',
  ...
  watch: {
    '$route.path'(val) {
      if (val == '/gyhh/gyhhfxbg' && this.$store.getters.isHome === true) {
  ...
  methods: {
      ...
      try {
        if (this.reportType == 'preview') {
          self.iframeData.src = this.file_vmrisk_src_preview(
            ...
            '8404dfce-7455-4da7-8282-760b8a99de2c',
    ...
    file_vmrisk_src_view(templateCode, beginDate, endDate, fundCode, benchmarkA, benchmarkB, benchmarkAValue, benchmarkBValue, kpiCode, contrasCode,desensitizationFlag,desensitizationFun) {
      return (
        process.env.BASE_API +
        '/rest/report/interview.do?templateCode=' +
        templateCode +
    ...
    fun_export_PDF() {
      ...
      const templateCode =  '8404dfce-7455-4da7-8282-760b8a99de2c'
      const pdfName = '股票多头.pdf'
      ...
      //务时封面
      const covIndexCode='9adb8c7d-3fb0-42ae-9baa-4ecb71c95c17'
