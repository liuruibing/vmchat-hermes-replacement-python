...
<!--                    <el-form-item label="" prop="fundName">
                    ...
                    <el-form-item label="对比指数">
                      <el-select v-model="formInline.contrasCode" size="small" clearable filterable placeholder="请选择指数">
                        <el-option v-for="item in factorIndexList" :key="item.dimCde" :label="item.dimNme" :value="item.dimCde" />
                      </el-select>
                    </el-form-item>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      //开关
      offOn: false,
      name: 'iframeVue_reportGF_FACTORINDEX',
      id: 'J_iframe_id_reportGF_FACTORINDEX',
      baseApi: '', // 页面根地址
      exportButton: false,
      exportText: '导出',
  ...
  mounted() {
    ...
    //初始化多因子的对比指数
    this.getFactorIndexEdims("FACTORINDEX")
  },
  watch: {
    '$route.path'(val) {
      if (val == '/jxbg/factorindex' && this.$store.getters.isHome === true) {
  ...
  methods: {
    ...
    // 多因子对比指数列表
    getFactorIndexEdims (type) {
      // 字典类型
      let params = type
      commonApi.edimsSelectType(params).then(res => {
        let { status, data } = res.data
        if (status === 0) {
          this.factorIndexList = data
        }
      })
    },
      ...
      try {
        if (this.reportType == 'preview') {
          self.iframeData.src = this.file_vmrisk_src_preview(
            ...
            'c7ef520a-78a3-48cb-9dd4-d40b3e23045f',
    ...
    file_vmrisk_src_view(templateCode, beginDate, endDate, fundCode, benchmarkA, benchmarkB, benchmarkAValue, benchmarkBValue,contrasCode,freq,desensitizationFlag,desensitizationFun) {
      return (
        process.env.BASE_API +
        '/rest/report/interview.do?templateCode=' +
        templateCode +
    ...
    fun_export_PDF() {
      ...
      const templateCode =  'c7ef520a-78a3-48cb-9dd4-d40b3e23045f'
      const pdfName = '标准报告.pdf'
