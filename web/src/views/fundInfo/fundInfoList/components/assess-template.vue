<template>
  <div class="assess-template">
    <el-form ref="formData" :model="formData" :rules="rules" class="standard-form" style="margin: 0 auto" label-width="auto">
      <el-form-item label="估值文件模板" prop="templateId">
        <el-select class="template" v-model="formData.templateId" size="small" filterable placeholder="请选择">
          <el-option v-for="item in tempateOptions" :key="item.fId" :label="item.templateName" :value="item.fId" />
        </el-select>
      </el-form-item>
      <el-form-item label="匹配规则" prop="matchRule">
        <el-input v-model="formData.matchRule" placeholder="请输入" type="textarea"></el-input>
      </el-form-item>
    ...
    <div style="text-align: center" class="mt20 standard-form">
      <el-button size="small" type="primary" @click="handleSaveAssess">提 交</el-button>
...
<script>
import { updateTemplate } from '../scripts/api'
export default {
  name: 'assessTemplate',
  props: {
    tempateOptions: {
    ...
    curAssessRow: {
      handler(val) {
        if (val && val.fundCode) {
          ...
          this.formData.matchRule = val.matchRule || ''
        } else {
          this.$message({
            type: 'error',
            message: '缺少产品代码参数',
  ...
  data() {
    return {
      ...
      rules: {
        ...
        matchRule: [{ required: true, message: '匹配规则不能为空', trigger: 'blur' }]
      }
    }
  },
  methods: {
