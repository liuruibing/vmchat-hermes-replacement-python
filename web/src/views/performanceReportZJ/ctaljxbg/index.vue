                ...
                <el-col :span="24">
                  ...
                  <div v-show="formInline.ZB_INDEXTYPE == 'fhjz'" style="display: inline-block">
                    ...
                    <el-form-item label="基准 A">
                      <el-select v-model="formInline.ZB_INDEXCODEA" clearable placeholder="请选择" size="small">
                        <el-option v-for="item in $store.state.edims.datumOption" :key="item.id" :label="item.label" :value="item.id"></el-option>
                      </el-select>
                    </el-form-item>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      formInline: {
        ...
        ZB_INDEXCODEA: 'NH0100', // 基准A
        ZB_INDEXWEIGHTA: '0.0000', // 基准A权重
        ZB_INDEXCODEB: 'NH0100', // 基准B
      ...
      //开关
      offOn: false,
      name: 'iframeVue_ReportZJCTALJXBG',
      id: 'J_iframe_id_ReportZJCTALJXBG',
      baseApi: '', // 页面根地址
      exportButton: false,
      onButton:false,
      exportText: '导出',
  ...
  watch: {
    '$route.path'(val) {
      if (val == '/jxbg/ctaljxbg' && this.$store.getters.isHome === true) {
        this.$store.commit('SET_HOME_STATUS', false)
        var self = this
        let beg = self.$route.query.rows.beginDate
        let end = self.$route.query.rows.endDate
    ...
    'formInline.beginDate'(newVal, oldVal) {
      var self = this
      if (newVal !== oldVal) {
        self.getTimeBig()
      }
    ...
    'formInline.fundCode'(newVal, oldVal) {
      var self = this
      if (newVal !== oldVal) {
        self.getTimeBig()
        self.getTimeEnd()
  ...
  methods: {
    ...
    fun_iframeSrc() {
      ...
      try {
        self.iframeData.src = this.file_vmrisk_src(
          ...
          '8577c119-27c2-4a55-b18d-ee7ad29d13b2',
          self.formInline.beginDate,
          self.formInline.endDate,
    ...
    fun_export_PDF() {
      ...
      const templateCode = '8577c119-27c2-4a55-b18d-ee7ad29d13b2'
      const wordName = 'CTA类绩效报告.pdf'
