    ...
    <el-form ref="queryForm" :inline="true" class="standard-form" :model="clsjSection" :rules="searchRules" label-position="left" size="small">
      ...
      <el-row class="tzcl">
        <el-form-item label="投资策略" label-width="70px" style="display: flex; align-items: center">
          <span class="tzclbx" style="font-size: 14px; padding: 4px 10px" @click="handleBxBtn('.tzclbx')">不限</span>
          <span v-for="ele in investStrategy" :key="ele.id" :class="ele.checked ? 'itemBox-checked' : 'itemBox'">
            <el-checkbox v-model="ele.checked" :label="ele.label" @change="handleCheckedChange($event, '.tzclbx', ele)" />
          </span>
        </el-form-item>
      </el-row>
      <!--  运行状态 / 是否分级 / 净值频率 / 绩效评级 -->
      <el-row class="jzpl">
        ...
        <el-form-item class="jxpjitem" label="绩效评级" label-width="70px" style="display: flex; align-items: center">
          <div v-for="e in jxpjOptions" :key="e.mid" style="display: flex; align-items: center">
            <el-select v-model="e.selected" :disabled="switchPageType === '2'" :placeholder="e.placeholder" size="small" style="margin-right: 15px" @change="jxpjChangeSelect">
              <el-option v-for="item in e.selectOptions" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </div>
        </el-form-item>
      </el-row>
