<template>
  ...
  >
    ...
    <el-row>
      <el-col :span="24">
        <el-form style="width: 100% !important" ref="editForm" :inline="true" :model="formQuery" :rules="rules" class="standard-form" label-width="auto">
      ...
      <el-col style="text-align: center" class="standard-form">
        <el-button type="primary" size="small" @click="savePool('editForm')">提 交</el-button>
      </el-col>
    </el-row>
  </el-dialog>
</template>
...
<script>
import { saveFundPool, renameFundPool } from '../scripts/api'
export default {
  name: 'GroupEdit',
  props: {
    currentData: {
    ...
    savePool(formName) {
      this.$refs[formName].validate(async (valid) => {
        if (valid) {
          if (this.status === 'ADD') {
            try {
              const res = await saveFundPool(this.formQuery)
            ...
            try {
              const res = await renameFundPool(data)
              if (res.data.status === 0) {
                this.$message({
                  type: 'success',
