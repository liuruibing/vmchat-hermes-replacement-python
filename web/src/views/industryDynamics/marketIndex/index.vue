...
export default {
  ...
  data() {
    return {
      ...
      typeToSqlCode: {
        ...
        jqbx: {
          jzrq: 'ceb5fd06-cc28-4f61-abec-563cd39290a8',
          table: '47442f7b-fe3f-45a9-8bbf-982c42a02ec1'
        },
      },
      kpiOptions: {},
      returnOptions: {},
  ...
  methods: {
    ...
    async init() {
      ...
      // 获取业绩走势的echarts数据
      const yjzsReturn = await this.getEchartsData(this.typeToSqlCode[pageType].yjzs)
      this.handleYJZSEcharts(yjzsReturn.data)

      // 获取动态回撤的echarts数据
      const dthcReturn = await this.getEchartsData(this.typeToSqlCode[pageType].dthc)
    ...
    async handleJQBXjzrq(date){
      ...
      const jqbxTableReturn = await this.getTableData(this.typeToSqlCode.jqbx.table,this.IndustryDataUpDate)
    ...
    exportYjzsToExcel() {
      ...
      try {
        ...
        this.exportToExcel(excelData, '业绩走势', '广发私募指数_业绩走势数据');
      } catch (error) {
        this.exportLoading = false;
    ...
    exportDthcToExcel() {
      ...
      try {
        ...
        // 4. 合并表头和数据行，调用通用导出方法
        const excelData = [header, ...dataRows];
        this.exportToExcel(excelData, '动态回撤', '广发私募指数_动态回撤数据');
      } catch (error) {
        this.exportLoading = false;
