      ...
      <el-form :inline="true" :model="formQuery" class="standard-form">
        ...
        <el-form-item class="dbjz">
          <el-select v-model="formQuery.indexType" clearable collapse-tags multiple placeholder="请选择指数类型" size="small">
            <el-option v-for="item in styleIndexOptionsProps" :key="item.DIM_CDE" :label="item.DIM_NME + '(' + item.DIM_CDE + ')'" :value="item.DIM_CDE"></el-option>
          </el-select>
        </el-form-item>
...
<!--            <el-button
        ...
        <div class="chart-content" v-loading="searchLoading">
          <myChart id="yjzsStyleIndex" ref="yjzsStyleIndexRef" :options="kpiOptions" class="echarts_box" resize-dom="cmbfof" />
        </div>
      </div>
...
<!--            <el-button
        ...
        <div class="chart-content" v-loading="searchLoading">
          <myChart id="dthcStyleIndex" ref="dthcStyleIndexRef" :options="returnOptions" class="echarts_box" resize-dom="cmbfof" />
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    async init() {
      ...
      // 获取业绩走势的echarts数据
      const yjzsReturn = await this.getEchartsData(this.styleIndex.yjzs)
      this.handleYJZSEcharts(yjzsReturn.data)

      // 获取动态回撤的echarts数据
      const dthcReturn = await this.getEchartsData(this.styleIndex.dthc)
      this.handleDTHCEcharts(dthcReturn.data)

      //动态获取近期表现数据
      const jqbxJzrqReturn = await this.getEchartsData(this.styleIndex.jqbxjzrq)
    ...
    async handleJQBXjzrq(date){
      ...
      const jqbxTableReturn = await this.getTableData(this.styleIndex.jqbxtable,this.IndustryDataUpDate)
    ...
    exportYjzsToExcel() {
      ...
      try {
        ...
        this.exportToExcel(excelData, '业绩走势', '风格指数_业绩走势数据');
      } catch (error) {
        this.exportLoading = false;
    ...
    exportDthcToExcel() {
      ...
      try {
        ...
        // 4. 合并表头和数据行，调用通用导出方法
        const excelData = [header, ...dataRows];
        this.exportToExcel(excelData, '动态回撤', '风格指数_动态回撤数据');
      } catch (error) {
        this.exportLoading = false;
