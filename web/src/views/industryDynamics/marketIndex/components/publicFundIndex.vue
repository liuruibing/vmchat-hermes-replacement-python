      ...
      <el-form :inline="true" :model="formQuery" class="standard-form">
        ...
        <el-form-item class="dbjz">
          <el-select v-model="formQuery.indexType" clearable collapse-tags multiple placeholder="请选择指数类型" size="small">
            <el-option v-for="item in publicFundIndexOptionsProps" :key="item.DIM_CDE" :label="item.DIM_NME + '(' + item.DIM_CDE + ')'" :value="item.DIM_CDE"></el-option>
          </el-select>
        </el-form-item>
...
<!--            <el-button
        ...
        <div class="chart-content" v-loading="searchLoading">
          <myChart id="yjzsPublicFundIndex" ref="yjzsPublicFundIndexRef" :options="kpiOptions" class="echarts_box" resize-dom="cmbfof" />
        </div>
      </div>
...
<!--            <el-button
        ...
        <div class="chart-content" v-loading="searchLoading">
          <myChart id="dthcPublicFundIndex" ref="dthcPublicFundIndexRef" :options="returnOptions" class="echarts_box" resize-dom="cmbfof" />
        </div>
      </div>
    </div>
  </div>
...
export default {
  name: 'PublisFundIndex',
  components: {
    myChart
  },
  props: {
    publicFundIndexOptionsProps: {
      type: Array,
      default: () => {
        return []
      }
  ...
  methods: {
    ...
    async init() {
      ...
      // 获取业绩走势的echarts数据
      const yjzsReturn = await this.getEchartsData(this.publicFundIndex.yjzs)
      this.handleYJZSEcharts(yjzsReturn.data)

      // 获取动态回撤的echarts数据
      const dthcReturn = await this.getEchartsData(this.publicFundIndex.dthc)
      this.handleDTHCEcharts(dthcReturn.data)

      //动态获取近期表现数据

      const jqbxJzrqReturn = await this.getEchartsData(this.publicFundIndex.jqbxjzrq)
    ...
    async handleJQBXjzrq(date){
      ...
      const jqbxTableReturn = await this.getTableData(this.publicFundIndex.jqbxtable,this.IndustryDataUpDate)
    ...
    exportYjzsToExcel() {
      ...
      try {
        ...
        this.exportToExcel(excelData, '业绩走势', '公募基金指数_业绩走势数据');
      } catch (error) {
        this.exportLoading = false;
    ...
    exportDthcToExcel() {
      ...
      try {
        ...
        // 4. 合并表头和数据行，调用通用导出方法
        const excelData = [header, ...dataRows];
        this.exportToExcel(excelData, '动态回撤', '公募基金指数_动态回撤数据');
      } catch (error) {
        this.exportLoading = false;
