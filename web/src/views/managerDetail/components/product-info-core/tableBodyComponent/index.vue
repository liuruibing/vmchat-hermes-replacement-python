        ...
        <template slot-scope="scope">
          ...
          <span v-else>
            <span v-if="isColorStr(item.label) && item.filter == 'numberToFixedStr'">
              <span v-if="Number(scope.row[item.id]) > 0 && item.label.indexOf('最大回撤') < 0" style="color: red">
              ...
              <span v-else-if="Number(scope.row[item.id]) < 0 || item.label.indexOf('最大回撤') >= 0" style="color: green">
                <span v-if="numToFixedNoStrArray.includes(item.id)">
                  {{ scope.row[item.id] | numberToFixedStr(2, 1) }}
                </span>
                <span v-else>{{ scope.row[item.id] | numberToFixedStr(2, 100, '%') }}</span>
            ...
            <span v-else-if="item.filter == 'numberToFixedStr4'">
              <span v-if="Number(scope.row[item.id]) > 0" style="color: red">
                <span v-if="numToFixedNoStrArray.includes(item.id)">
                  {{ scope.row[item.id] | numberToFixedStr(4, 1) }}
                </span>
                <span v-else>{{ scope.row[item.id] | numberToFixedStr(4, 100, '%') }}</span>
              </span>
              <span v-else-if="Number(scope.row[item.id]) < 0" style="color: green">
                <span v-if="numToFixedNoStrArray.includes(item.id)">
                  {{ scope.row[item.id] | numberToFixedStr(4, 1) }}
                </span>
                <span v-else>{{ scope.row[item.id] | numberToFixedStr(4, 100, '%') }}</span>
            ...
            <span v-else-if="item.filter == 'echarts'">
              <myecharts
                :id="scope.row.fundCode || scope.row.FUNDCODE"
                :ref="scope.row.fundCode || scope.row.FUNDCODE"
  ...
  methods: {
    ...
    handleTableHeight() {
      setTimeout(() => {
        ...
        const baseInfo = document.querySelector('.base-info').offsetHeight
        const diff = appMain - baseInfo - 39 - 30 - 46 - 10 - 35 - 42 - 20 - 18
        if (diff < 300) {
    ...
    jumpPage(row) {
      if ((row.isPermission && row.isPermission == '0') || (row.isStatus && row.isStatus == '1')) {
        ...
        this.$message.warning('此产品业绩信息暂时不可查看')
        return
      }
