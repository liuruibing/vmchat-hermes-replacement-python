<template>
  <div>
    <el-dialog :visible.sync="dialogVisible" style="padding: 0px 20px;" width="80%" @closed="handleDialogClosed">
      <el-row :gutter="21" justify="center" type="flex">
        <el-col :span="5" style="border: 1px solid #cccccc;padding: 0px;max-height: 262px;overflow-y: auto;">
          <el-tree
            ref="leftTreeData"
            :data="leftTreeData"
            :default-checked-keys="['fundCode', 'fundName']"
      ...
      <div style="padding-top: 20px">
        <el-table ref="table" :key="tableKey" :data="tableData" height="0px;" row-key="id" id="product-info-core-tablehead">
          <el-table-column v-for="(item, index) in tableHeaerList" :key="item.id" :label="item.label" :prop="item.id" align="center">
            <template slot="header" slot-scope="{ column, $index }">
              <span>{{ column.label }}</span>
              <i v-if="item.id !== 'fundCode' && item.id !== 'fundName' && item.id !== 'options'" class="el-icon-close" @click="handleDeleteColumn($index)"></i>
            </template>
          </el-table-column>
        </el-table>
      </div>
      <div style="color: #0F65DD;margin-top: 10px">*选中指标名称拖动调整顺序</div>
      <div slot="footer" class="dialog-footer">
        <el-button size="small" type="primary" @click="handleCancel">取 消</el-button>
...
import { centerTreeDataCenter, centerTreeDataRight, leftTreeData, rightTreeData } from './constant'
...
export default {
  ...
  methods: {
    ...
    columnDrop() {
      ...
      const wrapperTr = document.querySelector('#product-info-core-tablehead tr')
