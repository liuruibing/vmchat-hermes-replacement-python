<template>
   ...
   <div :style="{'height':form_show?'450px':'670px','overflow-y':form_show ? 'auto':'auto'}">
    <div style="width:80%;margin: 0 auto;">
            <el-steps  :active="1"   finish-status="success">
                ...
                <el-step  title="指令经办" description="当前处理">></el-step >
  ...
  <el-form label-position="right" label-width="110px" :inline="true" ref = "formInline" :model="formInline"  class="commitClass" style="margin-top: 8px;margin-bottom: 10px;">
            <el-row>
              <el-col :span="24">
      ...
      <el-form ref="formHandle" :model="formHandle" :rules="commitRules" v-show="form_show"   class="commitClass" auto-complete="on" label-position="left">
            <el-form-item label="处理意见" prop="remark">
             ...
             <el-button v-btn="pageData.cancleCode" style="margin-top:10px" :disabled="disabled_btn_abandon" v-if="abandon_btn_show" plain type="warning" size="medium" @click="updateStatus('formHandle','Cancle','撤销')">撤销</el-button>
...
import cmOptApi from '@/api/transfer/cmdOpt'
    ...
    methods: {
      ...
      updateStatus(vals,status,confirmStatus){
        this.$confirm('是否'+confirmStatus,'提示',{
              ...
              if(status === 'Retire'){
                  let followTasks = [{candidates:[],xmlId:'transfer_zljb'}];
                  taskExcuteCmdMode.followTasks = followTasks;
              }
              let taskId = this.myTaskId;
              cmOptApi.updateStatus(taskId,sendData).then(res => {
