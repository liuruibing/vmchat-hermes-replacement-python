<template>
  <div class="tracking_agentlist">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span v-if="scope.row.cmdType=='custom_demo'">自定义划款指令</span>
          <span v-else>{{ dataStatus(scope.row.cmdCode, "2") | noDataFilter }}</span>
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <span :class="scope.row.procStatus">{{
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            >查看详情</el-button
          >
          <!-- <el-button plain  v-if="isShowCl(scope.row)" v-show="cx_btn" type="danger" size="small" @click="serachDetail(scope.row,true)">处理</el-button> -->
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  created(){
    tableMethod.fetTableHeight(this,'trackingTable');
  },
  activated(){
    this.fetTableHeight();
    window.addEventListener("resize",this.fetTableHeight,false);
  },
  deactivated(){
    window.removeEventListener("resize",this.fetTableHeight,false);
  },
  mounted() {
    //初始化数据字典
    this.initHandle();
    ...
    //查询产品信息
    // this.getProductInfo();
    this.getEdimsGet("transfer.cmdCode");
  ...
  methods: {
    ...
    dataStatus(statusCode, flag) {
      if (flag === "0") {
        ...
        return this.cmdCodeTypeArr[statusCode];
      }
    },
    //金额千分位
    amountFilter(num) {
    ...
    initHandle() {
      let arr = [
        ...
        { name: "管理人经办", code: "managerOpt" },
        { name: "管理人经办", code: "transfer_glrjb" },
        { name: "管理人经办", code: "transfer_wzljb" },
        { name: "指令经办", code: "transfer_wcjdjb" },
    ...
    getEdimsGet(val) {
      ...
      commonApi.getEdims(params).then((res) => {
        if (res.data.message === "success") {
          let arr = res.data.data;
          let arr1 = [
            { dimNme: "交易所交易（付款）", dimCde: "gm040601" },
            { dimNme: "交易所交易（收款）", dimCde: "gm040602" },
            { dimNme: "备付金代付", dimCde: "gm040603" },
            { dimNme: "托管转中债", dimCde: "gm040701" },
            { dimNme: "中债转托管", dimCde: "gm040702" },
            { dimNme: "托管转上清", dimCde: "gm040703" },
            { dimNme: "上清转托管", dimCde: "gm040704" },
            { dimNme: "中债转上清", dimCde: "gm040705" },
            { dimNme: "上清转中债", dimCde: "gm040706" },
          ]
          let arr2 = [...arr,...arr1]
          for (let i = 0; i < arr2.length; i++) {
            // this.cmdCodeTypeArr[arr2[i].dimCde] = arr2[i].dimNme;
            this.$set(this.cmdCodeTypeArr,arr2[i].dimCde,arr2[i].dimNme);
          }
          
          // this.cmdCodeTypeArr = [...this.cmdCodeTypeArr,...arr1]
          // console.log(this.cmdCodeTypeArr);
        } else {
          this.$message.closeAll();
