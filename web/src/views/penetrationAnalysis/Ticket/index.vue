...
<!--              <el-form-item label="" prop="vcFundcode">
              ...
              <el-form-item label="" prop="dimension">
                <el-select v-model="formQuery.dimension" placeholder="请选择展示维度" size="small">
                  <el-option v-for="item in typeOptions1" :key="item.value" :label="item.label" :value="item.value"></el-option>
                </el-select>
              </el-form-item>

              <el-form-item label="" prop="secType">
                <el-select v-model="formQuery.secType" placeholder="请选择证券类别" size="small">
                  <el-option v-for="item in hyOptions" :key="item.CODE" :label="item.DISPLAY" :value="item.CODE"></el-option>
                </el-select>
              </el-form-item>
              <el-form-item label="" prop="symbolCode">
                <el-input v-model="formQuery.symbolCode" placeholder="请输入证券代码" size="small"></el-input>
              </el-form-item>
              <el-form-item label="" prop="symbolName">
                <el-input v-model="formQuery.symbolName" placeholder="请输入证券名称" size="small"></el-input>
              </el-form-item>
              <el-form-item class="">
    ...
    <div class="chart-main">
      <div class="target-trend">
        <div class="page-model-title">
          ...
          <span class="title-txt">穿透分析-个券</span>
        </div>
...
export default {
  ...
  data() {
    ...
    return {
      formQuery: {
        ...
        unit: '',
        vcdata: 'FCBPRICE',
        //conditions:['产品代码#日期#计算方式#展示维度#'],
        vcFundname: '',
        conditions: ['产品代码#', '日期#', '计算方式#', '展示维度#', '证券代码#', '证券名称#', '证券类别#'],
        fId: '',
        vcParentFundcode: '0',
        vcFundcode: '',
        dimension: 'sz',
  ...
  methods: {
    ...
    vcDatas() {
      if (this.formQuery.calculateType == 'cb' && this.formQuery.dimension == 'sz') {
        this.formQuery.vcdata = 'FCBPRICE'
      } else if (this.formQuery.calculateType == 'cb' && this.formQuery.dimension == 'sl') {
        this.formQuery.vcdata = 'FCBAMOUNT'
      } else if (this.formQuery.calculateType == 'sz' && this.formQuery.dimension == 'sz') {
        this.formQuery.vcdata = 'FSZPRICE'
      } else if (this.formQuery.calculateType == 'sz' && this.formQuery.dimension == 'sl') {
        this.formQuery.vcdata = 'FSZAMOUNT'
      }
    },
    //查询获取数据
    handleQueryChartData() {
      ...
      try {
        const params = {
          ...
          typeName: this.formQuery.typeName || '',
          ...
          industry: this.formQuery.industry || '',
          xtwoname: this.formQuery.xtwoname || '',
          xthreename: this.formQuery.xthreename || '',
          symbolCode: this.formQuery.symbolCode || '',
          symbolName: this.formQuery.symbolName || '',
          secType: this.formQuery.secType || '',
          dimension: this.formQuery.dimension
        }
        request({
          url: `/api/fundanalyze/analyzeFundSymbol.do`,
          method: 'post',
          data: params
        }).then((res) => {
          if (res.status === 200) {
    ...
    dataOrganization() {
      ...
      if (data2.length > 0) {
        ...
        for (var i = 0; i < data3.length; i++) {
          this.tableColumns.push({ prop: data3[i], label: data4[i], align: 'center' })
        }
        const specialLabels = ['证券代码', '证券名称', '市场', '母基金公允价值合计(元)', '占母基金净资产比例(%)', '占母基金总资产比例(%)']

        if (this.tableColumns.length > 0) {
          const data5 = this.tableColumns.filter((column) => specialLabels.includes(column.label))
          const data6 = this.tableColumns.filter((column) => !specialLabels.includes(column.label))
          this.tableColumns = [...data5, ...data6]
        }
    ...
    exportData() {
      ...
      const fileName = '个券-导出.xls'
      const url = 'api/fundanalyze/exportSymbol.do?searchMod=download'
