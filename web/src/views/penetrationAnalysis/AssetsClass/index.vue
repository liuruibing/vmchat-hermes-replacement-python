    ...
    <div class="chart-main">
      <div class="target-trend">
        <div class="page-model-title">
          ...
          <span class="title-txt">穿透分析-资产类别</span>
        </div>
    ...
    dataOrganization() {
      ...
      if (data2.length > 0) {
        ...
        for (var i = 0; i < data3.length; i++) {
          if (data4[i] != '维度名称' && data4[i] != '占母基金净资产比例(%)' && data4[i] != '占母基金总资产比例(%)' && data4[i] != '母基金公允价值合计(元)') {
            this.tableColumns.push({ prop: data3[i], label: data4[i] + '(元)', align: 'center' })
          } else {
            this.tableColumns.push({ prop: data3[i], label: data4[i], align: 'center' })
          }
        }
        if (this.tableColumns.length > 0) {
          const data5 = this.tableColumns.filter((column) => ['维度名称', '母基金公允价值合计(元)', '占母基金净资产比例(%)', '占母基金总资产比例(%)'].includes(column.label))
          const data6 = this.tableColumns.filter((column) => !['维度名称', '母基金公允价值合计(元)', '占母基金净资产比例(%)', '占母基金总资产比例(%)'].includes(column.label))
