...
<!--              <el-form-item label="" prop="vcFundcode">
              ...
              <el-form-item label="" prop="industry">
                <el-select v-model="formQuery.industry" placeholder="请选择行业" size="small">
    ...
    <div class="chart-main">
      <div class="target-trend">
        <div class="page-model-title">
          ...
          <span class="title-txt">穿透分析-行业</span>
        </div>
...
export default {
  ...
  data() {
    ...
    return {
      formQuery: {
        ...
        vcFundname: '',
        conditions: ['产品代码#', '日期#', '计算方式#', '展示维度#', '行业#'],
        fId: '',
        vcParentFundcode: '0',
        vcFundcode: '',
        dimension: 'sz'
  ...
  methods: {
    ...
    handleTableHeight() {
      this.$nextTick(() => {
        // 根据浏览器高度设置初始高度
        this.tableHeight = window.innerHeight - this.$refs.refsTable.$el.offsetTop - 140
        // 监听浏览器高度变化，修改表格高度
        window.onresize = () => {
          this.tableHeight = window.innerHeight - this.$refs.refsTable.$el.offsetTop - 140
        }
    ...
    dataOrganization() {
      ...
      if (data2.length > 0) {
        ...
        if (this.tableColumns.length > 0) {
          const data5 = this.tableColumns.filter(
            (column) =>
              column.label === '维度名称' || column.label === '母基金公允价值合计(元)' || column.label === '占母基金净资产比例(%)' || column.label === '占母基金总资产比例(%)'
          )
          const data6 = this.tableColumns.filter((column) => !data5.includes(column))
