...
<template>
  <div class="tempConfig-wrapper">
    <el-dialog :visible.sync="visible" :title="dialogTitle" custom-class="dataBrowser-tempConfigDialog"
      ...
      <div class="tempConfig-container" :class="{ 'is-resizing': isDraggingLeft || isDraggingCenter }">
        ...
        <div class="tempConfig-right">
          <div class="tempConfig-title">
            模板预览
            <span style="color: red; font-size: 12px; font-weight: normal; margin-left: 20px">
              注意：操作左侧两个模块内容将重置模板表头顺序，建议先完成左侧两个模块的操作后，再拖拽调整模板表头顺序
            </span>
          </div>
    ...
    <!-- 指标维度设置弹窗 -->
    <indexParamSet ref="indexParamSetRef" v-if="indexParamSetShow" @close="indexParamSetShow = false"
      @submit="indexParamSetSubmit" />
  </div>
</template>
...
import indexParamSet from './indexParamSet.vue'
...
export default {
  ...
  methods: {
      ...
      try {
        ...
        if (status === 200 && colList) {
          ...
          indexParamsMap.forEach((params, vcId) => {
            ...
            // 设置已选的维度值，确保details和times是数组
            selectedItem.paramsValue = {
              details: Array.from(params.details),
              times: Array.from(params.times),
              unit: params.unit
            }
    ...
    handleClickItem(data) {
      ...
      this.indexParamSetShow = true
      this.$nextTick(() => {
        this.$refs.indexParamSetRef.init(data)
      })
    },
    // 指标维度设置弹窗提交
    indexParamSetSubmit(data) {
      // console.log('indexParamSetSubmit', data)
      const index = this.selectedIndexList.findIndex((item) => item.vcId === data.vcId)
      if (index !== -1) {
        // Vue2 中直接通过索引修改数组不具备响应式，改用 splice 触发视图和 computed 重新计算
        this.selectedIndexList.splice(index, 1, data)
      }
      this.updateTableDisplay()
    },
    // 拖拽结束
