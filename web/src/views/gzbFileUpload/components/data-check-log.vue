    ...
    <el-form  ref="formData"   class="standard-form" label-width="auto">
              ...
              <el-row>
                <el-col :span="24">
                  ...
                  <el-form-item label="估值日期">
                    <el-date-picker v-model="formInline.gzDate" placeholder="请选择估值日期" size="small" type="date" value-format="yyyy-MM-dd"></el-date-picker>
                  </el-form-item>

                  <el-form-item label="异常类型">
                    <el-select v-model="formInline.exception" size="small" clearable filterable placeholder="请选择异常类型">
                      <el-option v-for="item in exceptionOptionsList" :key="item.dimCde" :label="item.dimNme" :value="item.dimCde" />
                    </el-select>
                  </el-form-item>
                  <el-form-item>
      ...
      <el-row>
        <el-col :span="16">
          ...
          <span class="title-style">校验日志</span>
        </el-col>
        <el-col :span="8">
...
import {TABLE_DATA_CHECK_INFO_COLUMNS } from '../scripts/constants'
import { queryGzbDataCheckInfoLog,selectType } from '../scripts/api'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      // 表格数据
      // 表格列
      column: TABLE_DATA_CHECK_INFO_COLUMNS,
      // 分页信息
      pageList: {
        _pageSize: 20,
        _pageNum: 1,
  ...
  methods: {
     ...
     */
    initData() {
      this.getListCHECKINFOTYPEEdims('DIMGZCHECKINFOTYPE')
      // 查询表格
      this.handleSearchTable()
    },

      // 异常类型列表查询
     getListCHECKINFOTYPEEdims (type) {
        // 字典类型
        let params = type
        selectType(params).then(res => {
          let { status, data } = res.data
          if (status === 0) {
            this.exceptionOptionsList = data
          }
        })
      },
    /**
