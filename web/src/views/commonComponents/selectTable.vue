...
export default {
  ...
  mounted() {
   ...
   this.getProductInfo(this.defaultFirst)
   this.tableList = []
   this.tableListTemp = []
  },
  methods: {
   //查询产品信息
    getProductInfo(flag){
      ...
      bizBeginApi.getProductFund().then(({ data: res }) =>{
        if(res.status === 200){
          ...
          this.tableList = temp
          if(flag && temp.length){
            this.slValue = temp[0].codeName || ''
            this.$emit('sendDataObj', temp[0])
          }
    ...
    searchData (data) {
      ...
      let newData = this.tableListTemp.filter((itme) => {
        if (itme.codeName.toLowerCase().includes(data.toLowerCase())) return itme
      })
