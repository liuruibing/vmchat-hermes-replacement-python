      ...
      <div class="inner-margin">
        <el-form ref="formQuery" :inline="true" :rules="searchRules" :model="formQuery" class="standard-form">
          <el-row>
            ...
            <el-form-item label="" prop="shallRedeem">
              <span slot="label" class="item-label">申赎方向</span>
              <el-select v-model="formQuery.shallRedeem" clearable size="small" placeholder="请选择">
    ...
    <el-row>
      <el-col :span="16">
        <div class="page-model-title">
          ...
          <span class="title-txt">产品申赎查询</span>
        </div>
      </el-col>
      <el-col :span="8">
    ...
    <el-row id="standard-table">
      <div id="standard-table-margin">
        ...
        >
          ...
          <vxe-table-column field="SHALLREDEEM" title="申赎方向" min-width="100" header-align="center" align="center" />

          <vxe-table-column field="NETVALUE" title="单位净值（元）" min-width="120" header-align="center" align="right">
            <template slot-scope="scope">
              <span>{{ scope.row.NETVALUE | moneyYuan }}</span>
            </template>
          </vxe-table-column>
          <vxe-table-column field="QUOTIENT" title="份额(份)" min-width="150" header-align="center" align="right">
            <template slot-scope="scope">
              <span>{{ scope.row.QUOTIENT | moneyYuan }}</span>
            </template>
          </vxe-table-column>
...
import redemptionListAllApi from "@/api/comprehensivequery/redemptionListAll";
import FundListDialogMultiphase from '@/components/FundListDialogMultiphase';
import moment from 'moment'
import productSelector from '@/components/productSelector'

export default {
  name: "redemptionListAll",
  components: {
    Pagination,
    FundListDialogMultiphase,
    productSelector
  ...
  methods: {
    ...
    onQuery(flag) {
      ...
      let fileName="";
       if(params.fundCodes.length==1) {
        fileName=params.fundCodes[0]+"_"+params.fundNames+"_" +
         ...
         params.endDate.replace(/\-/g,"")+"产品申赎查询.xlsx";
       }else{
      ...
      if (flag) {
        ...
        params.ex_tmp_name = "产品申赎查询-导出.xlsx";
      }
      ...
      new Promise((resolve, reject) => {
        redemptionListAllApi.fun_selectredemptionListAll(params).then((res) => {
            console.log('查看数据')
            console.log(res)
