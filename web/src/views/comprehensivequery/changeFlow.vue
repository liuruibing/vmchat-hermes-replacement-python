        ...
        <el-form ref="formQuery" :inline="true" :rules="searchRules" :model="formQuery" class="standard-form">
          <el-row>
            <!-- <el-form-item label="" prop="beginDate" class="mb20">
              <span slot="label" class="item-label">开始时间</span>
              <el-date-picker
            ...
            <el-form-item label="" prop="endDate" class="mb20">
              <span slot="label" class="item-label">结束时间</span>
              <el-date-picker
            ...
            <el-form-item label="" prop="tradeDirection">
              <span slot="label" class="item-label">交易方向</span>
              <el-select class="jyfx" v-model="formQuery.tradeDirection" clearable size="small" placeholder="请选择">
            ...
            <el-form-item prop="symBolCode">
              <span slot="label" class="item-label">个券代码</span>
              <el-input v-model.trim="formQuery.symBolCode" clearable placeholder="请输入" size="small" />
            </el-form-item>
            <el-form-item prop="symBolName">
              <span slot="label" class="item-label">个券名称</span>
              <el-input v-model.trim="formQuery.symBolName" clearable placeholder="请输入" size="small" />
            </el-form-item>
            <el-form-item class="">
    ...
    <el-row>
      <el-col :span="16">
        <div class="page-model-title">
          ...
          <span class="title-txt">个券变动流水查询</span>
        </div>
      </el-col>
      <el-col :span="8">
    ...
    <el-row id="standard-table">
      <div id="standard-table-margin">
        ...
        >
          ...
          <vxe-table-column field="NEWDATE" title="日期" min-width="100" header-align="center" align="center" />
          <vxe-table-column field="COUPONCODE" title="个券代码" min-width="100" header-align="center" align="center" />
          <vxe-table-column field="COUPONNAME" title="个券名称" min-width="100" header-align="center" align="center" />

          <vxe-table-column field="AMOUNT" title="数量" min-width="120" header-align="center" align="right">
            <template slot-scope="scope">
              <span>{{ scope.row.AMOUNT | moneyYuan }}</span>
            </template>
          </vxe-table-column>
          <vxe-table-column field="UNIVALENCE" title="单价(元)" min-width="150" header-align="center" align="right">
            <template slot-scope="scope">
              <span>{{ scope.row.UNIVALENCE | moneyYuan }}</span>
            </template>
          </vxe-table-column>
          <vxe-table-column field="TRADEDIRECTION" title="交易方向" min-width="120" header-align="center" align="right"></vxe-table-column>
          <vxe-table-column field="TURNOVER" title="成交额(元)" min-width="120" header-align="center" align="right">
            <template slot-scope="scope">
              <span>{{ scope.row.TURNOVER | moneyYuan }}</span>
            </template>
          </vxe-table-column>
        </vxe-table>
        <vxe-pager
...
import changeFlowApi from "@/api/comprehensivequery/changeFlow";
...
export default {
  ...
  mounted() {
    ...
    if (self.$route.query.tradeDirection) {
      self.formQuery.tradeDirection.push(self.$route.query.tradeDirection);
    }
    this.$refs.table1.connect(this.$refs.tableToolbar);
    // 获取资产大类
    this.getListEdims("TD_TYPE");
    // 默认显示个框架
  ...
  methods: {
    ...
    onQuery(flag) {
      ...
      let fileName="";
       if(params.fundCodes.length==1) {
        fileName=params.fundCodes[0]+"_"+params.fundNames+"_" +
         ...
         params.endDate.replace(/\-/g,"")+"_个券变动流水查询.xlsx";
       }else{
      ...
      if (flag) {
        ...
        params.ex_tmp_name = "个券变动流水查询-导出.xlsx";
      }
      ...
      new Promise((resolve, reject) => {
        changeFlowApi.fun_selectChangeFlow(params).then((res) => {
            console.log('查看数据')
            console.log(res)
