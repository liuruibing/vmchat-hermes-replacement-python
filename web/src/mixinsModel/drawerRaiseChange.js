...
import pageApi from "@/api/productNew/releaseManage.js";
export default {
  ...
  methods: {
    ...
    serachSubmit(status, confirmStatus) {
        ...
        if (this.getDataForm && this.getDataForm.taskKey == 'changeRaise_ywfq') this.upNewFileUrl(sendData)
        else {
    ...
    upNewFileUrl (data) {
      ...
      pageApi.updateChangeRaise(formData).then(({data: res}) => {
        if (res.status == 200) {
          this.drawer = false;
          this.formHandle.remark = "";
