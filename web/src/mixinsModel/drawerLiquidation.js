...
import modelFileValidate from '@/mixinsModel/file-validate2'
import pageApi from "@/api/productNew/liquidation.js"
export default {
  ...
  methods: {
    ...
    handleChange(file, fileList) {
      this.fileValidate(file, fileList, 'fileList');
      this.onFreshValidation();
    },
    // 备案函
    ...
    handleChange2(file, fileList) {
      this.fileValidate(file, fileList, 'fileList2');
      this.onFreshValidation();
    },
    //查询综合办理业务类型
    getTransferType() {
    ...
    serachSubmit(status, confirmStatus) {
        ...
        if (myXmlId === "liquidation_ywfq" || (confirmStatus=='审批通过' && !this.back_show)) {
          let params = {};
        ...
        if (this.getDataForm && this.getDataForm.taskKey == 'liquidation_ywfq'){
          this.upNewFileUrl(sendData)
        }else if(confirmStatus=='审批通过' && !this.back_show){
          this.upNewFileUrl(sendData)
        }else {
    ...
    upNewFileUrl (data) {
      ...
      let fileList1 = this.formInline.fileList1
      for (let i of fileList1) {
        if (i.id) formData.append('fileId', i.id)
        else formData.append('file', i.raw);
      }
      let fileList2 = this.formInline.fileList2
      ...
      pageApi.updateLiquidationChange(formData).then(({data: res}) => {
        if (res.status == 200) {
          this.drawer = false;
          this.formHandle.remark = "";
