...
export default {
  ...
  methods: {
    ...
    serachSubmit(status, confirmStatus) {
        ...
        if (myXmlId === "contractChange_ywfq") {
          let params = {};
        ...
        if (this.getDataForm && this.getDataForm.taskKey == 'contractChange_ywfq') this.upNewFileUrl(sendData)
        else {
    ...
    upNewFileUrl (data) {
      ...
      pageApi.updateContractChange2(formData).then(({data: res}) => {
        if (res.status == 200) {
          this.drawer = false;
          this.formHandle.remark = "";
