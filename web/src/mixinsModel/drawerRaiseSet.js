...
export default {
  ...
  methods: {
    ...
    serachSubmit(status, confirmStatus) {
        ...
        if (this.getDataForm && this.getDataForm.taskKey == 'setRaise_ywfq') this.upNewFileUrl(sendData)
        else {
    ...
    upNewFileUrl (data) {
      ...
      pageApi.updateSetRaise(formData).then(({data: res}) => {
        if (res.status == 200) {
          this.drawer = false;
          this.formHandle.remark = "";
