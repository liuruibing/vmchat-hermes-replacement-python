...
import pageApi from "@/api/productNew/foundAndFiling.js"
export default {
  ...
  methods: {
    ...
    serachSubmit(status, confirmStatus) {
        ...
        if (this.getDataForm && this.getDataForm.taskKey == 'filing_ywfq') this.upNewFileUrl(sendData)
        else {
    ...
    upNewFileUrl (data) {
      ...
      pageApi.updateFilingChange(formData).then(({data: res}) => {
        if (res.status == 200) {
          this.drawer = false;
          this.formHandle.remark = "";
