<template>
  <div class="my-filter-excel">
    ...
    <div class="my-fe-footer">
      <el-button size="mini" plain type="primary" @click="confirmFilterEvent" content="确认">确认</el-button>
      <el-button size="mini" plain type="info" @click="resetFilterEvent" content="重置">重置</el-button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    load () {
      ...
      this.column = column
      this.option = option
      let colValList1 = colValList.filter(item=>{
        return item && item!="null"
      })
      this.colValList = colValList1
