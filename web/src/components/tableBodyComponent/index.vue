<template>
  <div id="standard-table">
    ...
    >
      <vxe-table-column v-if="showSelection" align="center" fixed="left" type="checkbox" width="55"></vxe-table-column>
      <vxe-table-column v-if="showFocus" align="center" fixed="left" title="关注" width="80">
          ...
          <span v-else>
            ...
            <span v-else-if="item.filter == 'fund'" class="color_4f95dd fund_cell" @click="jumpPage(scope.row)">
              <span class="fund_name">
                ...
                <span style="padding-left: 10px;"  v-if="scope.row['gfSelling']=='代销'">
                 <span class="greenGfSelling">代销</span>
               </span>
<!--              <img v-if="scope.row.vc_is_bd == '1' && item.id != 'companyName'" :src="require('@/assets/img/HOT.png')" height="14px" width="25px" />
...
<!--            <span v-else-if="item.id === 'companyName'" class="color_4f95dd fund_cell" @click="jumptomanager(scope.row)">
              <span class="fund_name">
                <el-tooltip :content="scope.row[item.id] || '-'" effect="dark" placement="top">
                  <span>{{ scope.row[item.id] | noDataFilter }}</span>
                </el-tooltip>
            ...
            <span v-else-if="item.filter == 'echarts'">
              <myecharts
                :id="(scope.row.fundCode || scope.row.FUNDCODE) + scope.row.source"
                :ref="(scope.row.fundCode || scope.row.FUNDCODE) + scope.row.source"
      ...
      <vxe-table-column v-if="$route.path.indexOf('/tqgl/wdgz') !== -1" align="center" fixed="right" title="操作" width="100px">
        <template slot-scope="scope">
          <span class="table-operation">
            <el-button type="info" size="small" plain @click="handleRemark(scope.row)">备 注</el-button>
          </span>
        </template>
      </vxe-table-column>
    </vxe-table>
  ...
  methods: {
    ...
    handleTableHeight() {
      setTimeout(() => {
        const appMain = document.getElementById('appMain').offsetHeight
        const ddQuery = document.getElementById('dd-query').offsetHeight
        const diff = appMain - ddQuery - 45 - 38 - 32 - 10 - 10 - 60
        if (diff < 300) {
          this.tableHeight = 300
        } else {
          this.tableHeight = diff
    ...
    handleTableHeightMyFocus() {
      setTimeout(() => {
        const appMain = document.getElementById('appMain').offsetHeight
        const ddQuery = document.getElementById('dd-query').offsetHeight
        const diff = appMain - ddQuery - 29 - 4 - 35 - 42 - 18 - 70
        if (diff < 300) {
          this.tableHeight = 300
        } else {
          this.tableHeight = diff
