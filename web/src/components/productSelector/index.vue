<template>
  <el-dialog :visible.sync="productSelectDialogVisible" append-to-body width="80%" @close="handleDialogClose" v-auto-enter>
    <div style="font-size: 18px; color: #20201f; margin-bottom: 16px; text-align: center">产品选择</div>
    <el-row>
      <el-col :span="4" v-show="!leftList">
        <div class="left-dialog">
          <div class="left-title-bg">
            <span>{{ title }}</span>
            <span v-if="isShowButtonAndTag" style="margin-left: 10px">
              <el-tooltip class="item" content="可选产品为在产品结构管理页面已构建FOF树的FOF产品。" effect="dark" placement="top-start">
                <i class="el-icon-question"></i>
              </el-tooltip>
            </span>
            <el-button v-if="isShowButtonAndTag" size="small" type="primary" @click="handleFOFJump">去构建FOF树</el-button>
          </div>
          <el-tree
            ref="tree"
            class="standard-tree"
          ...
          >
            <span class="custom-node" slot="default" slot-scope="{ node, data }">
              <template v-if="data.label == '我的组合'">
                <span>{{ data.label }}</span>
                <el-tooltip class="item" content="我的组合分类下为资方同时作为管理人托管在广发的产品。" effect="dark" placement="top-start">
                  <i class="el-icon-question"></i>
                </el-tooltip>
              </template>
              <template v-else>
      ...
      <el-col :span="leftList ? 20 : 16" class="center-padding-left" v-if="multiple">
        <div class="center-dialog">
          <el-form :model="formQuery" class="center-form-bg standard-form">
            <el-input v-model="formQuery.productName" placeholder="基金简称/拼音/备案号" style="width: 180px !important" />
              ...
              <el-select v-model="formQuery.strategy" clearable placeholder="请选择策略类型">
                <el-option
      ...
      <el-col :span="leftList ? 24 : 20" class="center-padding-left" v-else>
        <div class="center-dialog">
          <el-form :model="formQuery" class="center-form-bg standard-form">
            <el-input v-model="formQuery.productName" placeholder="基金简称/拼音/备案号" style="width: 180px !important" />
            <el-select v-model="formQuery.strategy" clearable placeholder="请选择策略类型">
          ...
          <div id="standard-table">
            ...
            >
              <!-- 单选 -->
              <el-table-column align="center" header-align="center" width="55">
                <template slot-scope="scope">
                  <el-radio
                    v-model="radioSelected"
                    :label="scope.row.VC_FUNDCODE"
      ...
      <el-col :span="4" v-show="multiple">
        <div class="right-dialog">
          <div class="right-title-bg">
            <span style="padding-left: 10px">已选产品</span>
          ...
          <draggable v-model="selectListDialog" @end="dragging = false" @start="dragging = true">
            <div v-for="element in selectListDialog" :key="element.id" class="drag-element-style" style="line-height: 30px">
...
export default {
  ...
  methods: {
    ...
    handleFOFJump() {
      ...
      this.$nextTick(() => {
        this.$router.push({
          path: '/thgl/ctfx/cpjggl:id=601&jspPage=/api/masterfund/initListMasterFundPage.do'
        })
      })
    },
    ...
    judgeFlag(id) {
      // 判断是产品结构管理
      if (this.$route.path.includes('/thgl/ctfx/cpjggl')) {
        if (id == 'zstgwb') {
          return 'B'
        } else if (id == 'sycp') {
          return 'A'
      ...
      // 个券分析 债券分析单独处理 弹框权限
      else if (this.$route.path.includes('/thgl/ctfx/gqfx') || this.$route.path.includes('/thgl/ctfx/zqfx')) {
        if (id == 'zstgwb') {
          return 'F'
        } else if (id == 'wdgz') {
          return 'C'
      ...
      // 判断是投后的穿透页面的
      else if (this.$route.path.includes('/thgl/ctfx')) {
        if (id == 'zstgwb') {
          return 'D'
        } else if (id == 'wdgz') {
          return 'C'
      ...
      // 判断估值数据查询
      else if (this.$route.path.includes('/thgl/gzsjcx')) {
        if (id == 'zstgwb') {
          return 'B'
        } else if (id == 'wdgz') {
          return 'B'
      ...
      else if (this.$route.path.includes('/tqgl/smcpxx')) {
        if (id == 'zstgwb') {
          return 'B'
        } else if (id == 'wdgz') {
          return 'B'
      ...
      // 判断绩效数据查询
      else if (this.$route.path.includes('/thgl/jxsjcx')) {
        if (id == 'zstgwb') {
          return 'B'
        } else if (id == 'wdgz') {
          return 'B'
      ...
      } else if (this.$route.path.includes('/thgl/jxzb')) {
        if (id == 'zstgwb') {
          return 'B'
        } else if (id == 'wdgz') {
          return 'B'
      ...
      else if (this.$route.path.includes('/thgl/clpy/cpfgclpyjk')) {
        if (id == 'zstgwb') {
          return 'B'
        } else if (id == 'glrqxcp') {
          return 'A'
      ...
      }else if (this.$route.path.includes('/thgl/clpy/clpys')) {
        if (id == 'zstgwb') {
          return 'B'
        } else if (id == 'glrqxcp') {
          return 'A'
      ...
      else if (this.$route.path.includes('/thgl/tgfoffx')) {
        if (id == 'zstgwb') {
          return 'B'
        } else if (id == 'wdgz') {
          return 'B'
      ...
      // 判断定制报告
      else if (this.$route.path.includes('/ddportal/template')) {
        if (id == 'zstgwb') {
          return 'B'
        } else if (id == 'glrqxcp') {
          return 'A'
      ...
      // 判断是投中的准入二级菜单下的
      else if (this.$route.path.includes('/tzgl/zrlcgl')) {
        if (id == 'zstgwb') {
          return 'B'
        } else if (id == 'sycp') {
          return 'A'
      ...
      } else if (this.$route.path.includes('/tqgl/tc')) {
        if (id == 'zstgwb') {
          return 'B'
        } else if (id == 'scsm') {
          return ''
      ...
      // 判断需求互动
      else if (this.$route.path.includes('/xqhd/gxhxqgl')) {
        if (id == 'wdgz') {
          return 'B'
        } else if (id == 'zstgwb') {
          return 'B'
      ...
      // 判断是FOF子基金维护页面
      else if (this.$route.path.includes('/sjwh/fofzjjwh')) {
        if (id == 'glrqxcp') {
          return ''
        } else if (id == 'sycp') {
          return 'A'
      ...
      else if (this.$route.path.includes('/sjwh/gzbdr')) {
        if (id == 'glrqxcp') {
          return ''
        } else if (id == 'sycp') {
          return 'A'
    ...
    handleSelect(flag) {
      ...
      params.strategy = this.formQuery.strategy
    ...
    handleSameStatus() {
      if (!this.multiple) {
        ...
        if (this.selectListDialog.length > 0) {
          this.radioSelected = this.selectListDialog[0].VC_FUNDCODE;
        }
      } else {
        // 多选
        this.selectListDialog.forEach(function(item) {
          var row = this.tableData.find(function(sitem) {
            return sitem.VC_FUNDCODE === item.VC_FUNDCODE;
          });
    ...
    // 单选
    handleRadioSelect: function(row) {
      // 清空原有选中项
      this.selectListDialog = [];
      // 添加当前选中项
      this.selectListDialog.push(Object.assign({}, row));
      // 更新选中状态文本
      if (this.limit !== -1) {
        this.selectProductStr = '(' + this.selectListDialog.length + '/' + this.limit + ')';
      }
...
::v-deep .el-radio {
  ...
  text-align: center;
  .el-radio__label {
    display: none !important; // 隐藏单选按钮的文本标签
  }
  .el-radio__input {
    margin: 0 auto; // 按钮居中
  }
