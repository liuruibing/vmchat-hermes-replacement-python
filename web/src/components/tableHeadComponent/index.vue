<template>
  <div>
    <el-dialog :visible.sync="dialogVisible" style="padding: 0px 20px" width="80%" title="自定义指标" @closed="handleDialogClosed">
      <el-row :gutter="23" justify="center" type="flex" class="standard-form">
...
<!--        <el-col :span="6" style="border: 1px solid #cccccc; padding: 0px; border-left: none; max-height: 262px; overflow-y: auto">
      ...
      <!--  行列拖拽案例 -->
      <div style="padding: 20px 15px 0 15px" id="standard-table">
        <el-table
          ref="table"
          :key="tableKey"
          :cell-style="{ height: '50px', fontSize: '14px' }"
          :header-cell-style="{ color: '#333', height: '54px', background: '#f9fafe!important' }"
          :data="tableData"
          height="0px;"
          row-key="id"
        >
      ...
      <div style="color: #cf7e34; margin-top: 10px; margin-left: 15px">*选中指标名称拖动调整顺序</div>
      <div slot="footer" style="text-align: center" class="dialog-footer standard-form">
        ...
        <el-button type="info" plain size="small" @click="handleCancel">取 消</el-button>
      </div>
    </el-dialog>
  </div>
</template>
