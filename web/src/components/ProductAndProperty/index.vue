  ...
  <el-aside :width="boxWidth" class="ProductAndProperty_box"  >
    <div style="height: 100%;">
      <div :class="ProductAndPropertyData.productClass"  >
        <div  class="content_input" style="width: 260px;">
        ...
        <!--是否默认展开全部-->
        <!--:default-expand-all = ProductAndPropertyData.isProductDefaultExpandAll-->
        <el-tree
          ref="productTree"
          :data="productData"
          node-key="id"
          ...
          :default-expanded-keys="productExpandRowKeys"
      ...
      <!-- 长江特殊组合 -->
      <div :class="ProductAndPropertyData.specialFundcodeClass">
        <div class="content_input" style="width: 260px;">
          <el-select v-model="specialFundcodeVal" filterable clearable class="productSelectBox" placeholder="">
            <el-option
              v-for="(item, index) in specialFundcode"
              :key="item.dimCde"
              :label="item.dimNme"
              :value="item.dimCde">
            </el-option>
  ...
  export default {
    ...
    props: {
      ProductAndPropertyData: {
        ...
        propertyClass:'',//资产类名
        specialFundcodeClass: 'special_fundcode_none_box', // 长江特殊组合
        isEquity:'',//是否含权
        isManager:false,//是否是投资经理绩效详情页面
        tagsName:'',// 组合列表 tag子组件传值区分
        isNeedDefault:false,//是否需要默认选择第一个
        isProductDefaultExpandAll:true,//树状结构是否需要默认展开
        isProductLeftAll:true,//左侧组合列表是否需要默认展开
      },
    },
    data() {
      return {
        ...
        productExpandRowKeys:[],//产品列表默认展开一级
    ...
    mounted() {
      ...
      if(self.ProductAndPropertyData.productClass == "product_show_box" || self.ProductAndPropertyData.productClass == "content_average_box" ){
        ...
        //是否默认展开左侧组合列表
        if(self.ProductAndPropertyData.isProductLeftAll == false){
          self.ProductAndProperty_left_right();
        }
        else{
      ...
      // 长江特殊组合显示才发送请求
      if (this.ProductAndPropertyData.specialFundcodeClass === "special_fundcode_show_box") {
        // 长江特殊组合
        this.getListEdims('cj_special_fundcode')
      }
    ...
    methods: {
        ...
        new Promise((resolve, reject) => {
          commonApi.fundList(self.productSearch,self.ProductAndPropertyData.isEquity,self.manageVal).then(response => {
            ...
            if(response.data.data !== undefined && response.data.data != ''){
              ...
              if (flag) {
                // 默认展开所有
                self.productExpandRowKeys = fun.searchTreeData(responseData, 0, true).expandRowKeys;
              } else {
                // 默认展开一级
                self.productExpandRowKeys = fun.searchTreeData(responseData,0).expandRowKeys;
              }
                //需要默认选择
               if(self.ProductAndPropertyData.isNeedDefault == true){
                 //如果存在默认选中
      ...
      productMove(e){
        ...
        // console.log(e.clientX ,odiv.offsetLeft,1000);
      ...
      getListEdims (type) {
        ...
        commonApi.edimsCommon(params).then(res => {
          ...
          if (status === 0) {
            // 长江特殊组合
            if (type === 'cj_special_fundcode') {
              this.specialFundcode = data
              // 默认选中第一个
              if (data.length) this.specialFundcodeVal = data[0].dimCde
            }
          }
        })
      }
...
<style>
  ...
  /* 只显示长江特殊组合 */
  .ProductAndProperty_box .special_fundcode_none_box{
    display: none;
  }
  .ProductAndProperty_box .special_fundcode_show_box{
    height: 100%;
  }
