...
export default {
  ...
  methods: {
    ...
    handleMouseDownOnShape(e) {
      ...
      // this.curComponent.style.zIndex = '1000 !important'
      // console.log(this.curComponent.style.zIndex = '1000 !important')
      ...
      const editorEle = this.editor.getBoundingClientRect()
      // console.log('e', e)
      // console.log('pos', pos)
      // 如果元素没有移动，则不保存快照
      let hasMove = false
      const move = moveEvent => {
        ...
        if (pos.top >= (editorEle.height - pos.height)) {
          pos.top = editorEle.height - pos.height
        }
        pos.left = curX - startX + startLeft
        if (pos.left <= 0) {
          pos.left = 0
        }
        if (pos.left >= (editorEle.width - pos.width)) {
          pos.left = editorEle.width - pos.width
        }
    ...
    selectCurComponent(e) {
      ...
      if (this.$store.getters.curComponentIndex != this.$store.getters.componentData.length - 1) {
