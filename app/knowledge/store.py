...
class SqliteKnowledgeStore:
    """Compatibility knowledge store used when DuckDB is unavailable.

    It implements the same small interface as DuckDbKnowledgeStore, so the
    runtime keeps working everywhere. Installing DuckDB automatically upgrades
    the backend without changing agent code.
    """

    def __init__(self, path: str = ".runtime/knowledge.sqlite3") -> None:
        self.path = path
        if path != ":memory:":
