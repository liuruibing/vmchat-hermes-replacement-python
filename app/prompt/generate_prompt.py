...
def build_generate_prompt(
    ...
    system_prompt = "\n".join([
        ...
        "7. `catalog/modules/<moduleId>.md`：定位到唯一模块后读取该模块的展示字段映射、指标说明与样例；",
        ...
        "- 唯一定位模块后，必须读取对应 `catalog/profiles/<moduleId>.json`，再读取 `catalog/modules/<moduleId>.md`；",
        "- 涉及跨模块合并时必须读取 `skill/references/merge-guidance-v2.md`；旧版 `merge-rules.md` 仍可读取，但冲突时以 v2 为准；",
        "- 涉及子模块、DSL 或视图配置时按需读取相关 reference 或 submodules.json；",
