...
def build_generate_prompt(
    ...
    system_prompt = "\n".join([
        ...
        "",
        "# 资源读取策略",
        "- 若 search_agent_knowledge 工具可用，新建报表、指标定位和业务知识查询优先用自然语言搜索相关知识块，再读取命中的 Profile/模块资源；不要先把全量 metrics.md 塞进上下文。",
        "- 编辑已有报表时，先使用 currentDsls 和 selectedBlockId 判断目标；若已能唯一得到 moduleId，可直接读取对应 catalog/profiles/<moduleId>.json，不要为了流程形式重复读取全量指标表。",
        "- search_agent_knowledge 不可用或搜索证据不足时，再读取 catalog/index.md 与 catalog/profile-index.json 做粗定位；只有字段/指标名称存在歧义、Profile 信息不足或需要指标业务口径时，才读取 catalog/metrics.md。",
        ...
        "",
        "# 运行时覆盖规则（优先级高于上方 Skill 的旧流程）",
        "- 上方 Skill 中“必须先读取 index.md + metrics.md”“唯一定位后必须读取模块 MD”的旧资源路由被覆盖：编辑已有报表先用 currentDsls 定位并直读 Profile；新建/知识查询先读 index.md + profile-index；metrics.md 和模块 MD 仅在歧义或 Profile 证据不足时读取。",
        "- 上方 Skill 中运行 Node 脚本、写临时文件、调用 gen-uuid.js/validate-dsl.mjs 的要求被覆盖：当前模型没有这些工具，Python 服务端负责最终校验和修复循环。",
        "- 非 DSL 输出协议不得被 Skill 中“返回自然语言”的旧表述覆盖：业务说明必须 businessInfo JSON，澄清必须 clarify JSON，普通说明必须 chat JSON；禁止裸文本。",
