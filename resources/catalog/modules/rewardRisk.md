...
- 指标业务说明：
  ...
  - 年化收益率：统计区间内收益相对期初或投入规模的比例。
  - 基准收益率：统计区间内收益相对期初或投入规模的比例。
  - 年化波动率：“年化波动率”在该VM页面中的业务展示值。
  - 胜率：“胜率”在该VM页面中的业务展示值。
  - 最大回撤(复权)：“最大回撤(复权)”在该VM页面中的业务展示值。
  - 最大回撤(累计)：“最大回撤(累计)”在该VM页面中的业务展示值。
  - α系数：“α系数”在该VM页面中的业务展示值。
  - β系数：“β系数”在该VM页面中的业务展示值。
  - 夏普比率：“夏普比率”在该VM页面中的业务展示值。
  - 特雷诺比率：“特雷诺比率”在该VM页面中的业务展示值。
  - 信息比率：“信息比率”在该VM页面中的业务展示值。
  - 跟踪误差：“跟踪误差”在该VM页面中的业务展示值。
  - 索提诺比率：“索提诺比率”在该VM页面中的业务展示值。
  - 卡玛比率：“卡玛比率”在该VM页面中的业务展示值。
  - 欧米茄比率：“欧米茄比率”在该VM页面中的业务展示值。
  - 卡帕比率：“卡帕比率”在该VM页面中的业务展示值。
  - 上行捕获率：“上行捕获率”在该VM页面中的业务展示值。
  - 下行捕获率：“下行捕获率”在该VM页面中的业务展示值。
  - 下行标准差：“下行标准差”在该VM页面中的业务展示值。
...
- 指标候选：
  - 收益率（`body[].pyeild`）
  - 年化收益率（`body[].pyeild_year`）
  - 基准收益率（`body[].byeild`）
  - 年化波动率（`body[].pstd_year`）
  - 胜率（`body[].win_rate`）
  - 最大回撤(复权)（`body[].fq_maxdraw`）
  - 最大回撤(累计)（`body[].maxdraw`）
  - α系数（`body[].alpha`）
  - β系数（`body[].beta`）
  - 夏普比率（`body[].sharpe_year`）
  - 特雷诺比率（`body[].treynor_year`）
  - 信息比率（`body[].inforate_year`）
  - 跟踪误差（`body[].te_year`）
  - 索提诺比率（`body[].sortino_year`）
  - 卡玛比率（`body[].calmar_year`）
  - 欧米茄比率（`body[].omega_year`）
  - 卡帕比率（`body[].kappa_year`）
  - 上行捕获率（`body[].capture_up_year`）
  - 下行捕获率（`body[].capture_down_year`）
  - 下行标准差（`body[].pstd_pdwon_year`）
  - 上行捕获率（`body[].capture_up`）
  - 下行捕获率（`body[].capture_down`）
...
| body[].pyeild | pyeild | 收益率 | 原值直接使用 | % | table、echarts |
| body[].pyeild_year | pyeild_year | 年化收益率 | 原值直接使用 | % | table、echarts |
| body[].byeild | byeild | 基准收益率 | 原值直接使用 | % | table、echarts |
| body[].pstd_year | pstd_year | 年化波动率 | 原值直接使用 | % | table、echarts |
| body[].win_rate | win_rate | 胜率 | 原值直接使用 | % | table、echarts |
| body[].fq_maxdraw | fq_maxdraw | 最大回撤(复权) | 原值直接使用 | % | table、echarts |
| body[].maxdraw | maxdraw | 最大回撤(累计) | 原值直接使用 | % | table、echarts |
| body[].alpha | alpha | α系数 | 原值直接使用 | 未明确 | table、echarts |
| body[].beta | beta | β系数 | 原值直接使用 | 未明确 | table、echarts |
| body[].sharpe_year | sharpe_year | 夏普比率 | 原值直接使用 | 未明确 | table、echarts |
| body[].treynor_year | treynor_year | 特雷诺比率 | 原值直接使用 | 未明确 | table、echarts |
| body[].inforate_year | inforate_year | 信息比率 | 原值直接使用 | 未明确 | table、echarts |
| body[].te_year | te_year | 跟踪误差 | 原值直接使用 | 未明确 | table、echarts |
| body[].sortino_year | sortino_year | 索提诺比率 | 原值直接使用 | 未明确 | table、echarts |
| body[].calmar_year | calmar_year | 卡玛比率 | 原值直接使用 | 未明确 | table、echarts |
| body[].omega_year | omega_year | 欧米茄比率 | 原值直接使用 | 未明确 | table、echarts |
| body[].kappa_year | kappa_year | 卡帕比率 | 原值直接使用 | 未明确 | table、echarts |
| body[].capture_up_year | capture_up_year | 上行捕获率 | 原值直接使用 | 未明确 | table |
| body[].capture_down_year | capture_down_year | 下行捕获率 | 原值直接使用 | 未明确 | table |
| body[].pstd_pdwon_year | pstd_pdwon_year | 下行标准差 | 原值直接使用 | % | table、echarts |
| body[].capture_up | capture_up | 上行捕获率 | 原值直接使用 | 未明确 | echarts |
| body[].capture_down | capture_down | 下行捕获率 | 原值直接使用 | 未明确 | echarts |
